...
router.get('/:room', async (req, res) => {
  try {
    ...
    // ✅ since-based: नए + edited + deleted messages sync के लिए deleted को EXCLUDE नहीं करते
    if (req.query.since) {
      const query = { room, updatedAt: { $gt: new Date(req.query.since) } };
      const messages = await Message.find(query).sort({ createdAt: 1 }).limit(limit).lean();
      return res.json(messages);
    }
    // after (_id) based — legacy
    if (req.query.after) {
      const query = { room, deleted: { $ne: true } };
      try { query._id = { $gt: new mongoose.Types.ObjectId(req.query.after) }; } catch {}
      const messages = await Message.find(query).sort({ createdAt: 1 }).limit(limit).lean();
      return res.json(messages);
    }
    // Initial load — deleted messages को placeholder रखने के लिए include करते हैं
...
router.delete('/:room/:id', async (req, res) => {
  try {
    // deleted:true + content clear → updatedAt bump होगा → दूसरे devices sync कर लेंगे
