...
router.post('/', async (req, res) => {
  try {
    const { name, type, monthlyAmount, startDate, endDate, replaces, notes } = req.body;
    ...
    const entity = new SalaryEntity({
      ...
      endDate: endDate || null, replaces: replaces || null, notes: notes || '',
      active: !endDate,
    });
    await entity.save();

    // If replacing another entity, update that entity to mark it ended
    if (replaces) {
...
// DELETE entity (careful - also removes payments)
...
router.post('/seed', async (req, res) => {
  try {
    ...
    // Insert entities (skip if already exists)
    const entityResults = [];
    ...
    // Insert payments (avoid duplicates by name+date+amount)
