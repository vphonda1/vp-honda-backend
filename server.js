...
app.post('/api/push/send-reminder-summary', async (req, res) => {
  ...
  try {
    ...
    const Customer = mongoose.models.Customer || mongoose.models.customer;
    if (!Customer) {
      console.warn('[Cron] Customer model not found, sending generic push');
      const sent = await sendToAll('🔔 VP Honda Reminders', `${new Date().toLocaleDateString('en-IN')} — Open app to view today's reminders`, '/reminders');
      return res.json({ ok: true, sent, mode: 'no-model' });
    }
