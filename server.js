...
// ✅ NEW: सब reminders का summary push भेजें सब devices को
app.post('/api/push/send-reminder-summary', async (req, res) => {
  try {
    const total = await PushSub.countDocuments();
    if (total === 0) return res.status(400).json({ error: 'No devices subscribed' });

    // Fetch reminders from /api/reminders route OR compute here
    let summary = req.body.summary;
    if (!summary) {
      try {
        const baseUrl = `http://localhost:${PORT}`;
        const remRes = await fetch(`${baseUrl}/api/reminders`);
        const data   = await remRes.json();
        const overdue   = data.filter(r => r.status === 'overdue' || (r.daysOverdue && r.daysOverdue > 0)).length;
        const todayDue  = data.filter(r => r.status === 'due_today' || r.daysToDue === 0).length;
        const payment   = data.filter(r => r.type === 'payment').length;
        const service   = data.filter(r => r.type === 'service' || (r.type || '').toLowerCase().includes('service')).length;
        const insurance = data.filter(r => (r.type || '').toLowerCase().includes('insurance')).length;
        summary = { total: data.length, overdue, todayDue, payment, service, insurance };
      } catch (e) {
        console.warn('[Reminder summary] fetch failed:', e.message);
        summary = { total: 0, overdue: 0, todayDue: 0 };
      }
    }

    const title = `🔔 VP Honda Reminders`;
    const lines = [];
    if (summary.overdue)   lines.push(`🚨 ${summary.overdue} overdue`);
    if (summary.todayDue)  lines.push(`📅 ${summary.todayDue} due today`);
    if (summary.payment)   lines.push(`💳 ${summary.payment} payment`);
    if (summary.service)   lines.push(`🔧 ${summary.service} service`);
    if (summary.insurance) lines.push(`🛡️ ${summary.insurance} insurance`);
    const body = lines.length ? lines.join(' · ') : `${summary.total || 0} reminders pending. Open app to view.`;

    const sent = await sendToAll(title, body, '/reminders');
