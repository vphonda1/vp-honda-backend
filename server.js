...
const PushSubSchema = new mongoose.Schema({
  ...
  // ⭐ किस user का device है — इसी से "एक user के सारे phone/laptop पर
  //    notification" काम करता है. एक user के 3 device = 3 subscription,
  //    तीनों का userId एक.
...
// ⭐ एक ही user के सारे devices पर भेजो (assigned reminder के लिए)
async function sendToUser(userId, title, body, url, extra = {}) {
  const q = userId ? { userId } : {};
  let subs = await PushSub.find(q).lean();
  // उस user का कोई device registered नहीं? तो सबको भेज दो — reminder छूटने
  // से बेहतर है कि सबको पता चले.
  if (!subs.length && userId) subs = await PushSub.find().lean();
  const payload = JSON.stringify({
    title, body, url: url || '/reminders',
    icon:'/icons/icon-192x192.png', badge:'/icons/icon-96x96.png', ...extra,
  });
  let sent = 0;
  ...
  try {
    ...
    await PushSub.findOneAndUpdate(
      { endpoint: sub.endpoint },
      {
        ...
        deviceId:  sub.deviceId || '',
        // ⭐ किस user का device — assigned reminder इसी से सही आदमी को जाता है
        userId:    sub.userId   || '',
...
app.post('/api/push/send-reminder-summary', async (req, res) => {
  ...
  try {
    ...
    // ⚠️ FIX: पहले यहाँ 400 return होता था. GitHub Action इसे failure मानकर
    // 3 बार retry करता और आखिर में लाल cross के साथ fail हो जाता था.
    // कोई device subscribe न होना error नहीं है — इसलिए अब 200 भेजते हैं.
    if (total === 0) {
      return res.json({
        ok: true, sent: 0, mode: 'no-subscribers',
...
// ── SHOP SETTINGS alias ────────────────────────────────────────────────────────
// ⚠️ FIX: SalaryManagementPage `/api/shop-settings` call करता था जो कहीं था ही नहीं
// (हमेशा 404). असली route `/api/attendance/shop/settings` है. यह alias दोनों को
// काम करने देता है — पुराना कोई भी caller अब टूटेगा नहीं.
const ShopSettingsModel = require('./models/ShopSettings');
app.get('/api/shop-settings', async (req, res) => {
  try {
    const s = await ShopSettingsModel.findOne({ key: 'main' });
    res.json(s || {});
  } catch (err) { res.status(500).json({ error: err.message }); }
});
app.put('/api/shop-settings', async (req, res) => {
  try {
    const s = await ShopSettingsModel.findOneAndUpdate(
      { key: 'main' }, { ...req.body, key: 'main' }, { new: true, upsert: true }
    );
    res.json(s);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
// ⭐ POST /api/push/send-to-user — किसी एक user के सारे devices पर
// body: { userId, title, body, url }
app.post('/api/push/send-to-user', async (req, res) => {
  try {
    const { userId, title, body, url } = req.body || {};
    if (!title) return res.status(400).json({ error: 'title ज़रूरी है' });
    const out = await sendToUser(userId || '', title, body || '', url || '/reminders');
    res.json({ ok: true, ...out });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// ── PUSH HEALTH (diagnosis के लिए — browser में खोलकर देख सकते हैं) ──────────
app.get('/api/push/health', async (req, res) => {
  try {
    const total = await PushSub.countDocuments();
    const recent = await PushSub.find().sort({ savedAt: -1 }).limit(5)
      .select('deviceId userId userName userAgent savedAt').lean();
    const all = await PushSub.find().select('userId userName').lean();
    const byUser = {};
    all.forEach(x => {
      const k = x.userName || x.userId || '(अज्ञात)';
      byUser[k] = (byUser[k] || 0) + 1;
    });
    res.json({
      ok: true,
      devicesSubscribed: total,
      vapidPublicKeyTail: VAPID_PUBLIC.slice(-8),
      db: mongoose.connection.readyState === 1 ? 'connected' : 'disconnected',
      devicesPerUser: byUser,
      recent,
    });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
// ⭐ Visitors / Pickup-Drop / Appointments — पहले इनका डेटा सिर्फ़ phone के
//    localStorage में था और cache साफ़ होते ही खो जाता था. अब MongoDB में.
app.use('/api',                  require('./routes/tracking'));
// ⭐ MongoDB की जगह कहाँ भर रही है — browser में खोलकर देखने वाला tool
app.use('/api/storage',          require('./routes/storage'));
// ⭐ महीने-वार नफ़ा/नुक़सान — Excel की Summary sheet यहीं import होती है
app.use('/api/pnl',              require('./routes/pnl'));
// ⭐ हल्का "कुछ नया है?" endpoint — Dashboard की 11 request की जगह एक
